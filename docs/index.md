...
`memscore` is a mixed Python + R project for image memorability scoring
and reproducible benchmarking.

It has two user-facing layers:

- a Python CLI and API for scoring images and running benchmarks
- a native R wrapper package that can use either `reticulate` or the CLI
  backend

The package currently ships the released `ResMem` model as the default
scorer and includes benchmark workflows for simpler frozen CLIP
challengers.
...
Install the benchmark dependencies when you want frozen CLIP
challengers, LaMem benchmarking, or FIGRIM studies:
...
If you want to avoid the CLI path entirely, configure the R wrapper to
use `reticulate` instead:
...
- Python docs:
  [python-docs/index.rst](https://bbuchsbaum.github.io/memscore/python-docs/index.rst)
- R getting-started vignette:
  [vignettes/memscore.Rmd](https://bbuchsbaum.github.io/memscore/vignettes/memscore.Rmd)
- R benchmarking vignette:
  [vignettes/benchmarking.Rmd](https://bbuchsbaum.github.io/memscore/vignettes/benchmarking.Rmd)
- Benchmark guide:
  [BENCHMARKS.md](https://bbuchsbaum.github.io/memscore/BENCHMARKS.md)

## Repository Layout

- `memscore/`: public Python API and CLI
- `membench/`: Python benchmark engine
- `resmem_legacy/`: internal packaged copy of the released ResMem
  baseline code
- `R/`: native R wrapper package
- `vignettes/`: R package articles
- `python-docs/`: Sphinx documentation source for the Python and
...
See [BENCHMARKS.md](https://bbuchsbaum.github.io/memscore/BENCHMARKS.md)
for commands, manifest format, and study workflows.
