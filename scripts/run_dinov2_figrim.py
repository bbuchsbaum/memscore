"""Quick DINOv2 evaluation on FIGRIM — standalone script."""
...
def embed_dinov2(
    ...
    """Extract DINOv2 CLS-token features for a list of records."""
...
def main() -> None:
    figrim_root = Path("/Users/bbuchsbaum/code/memscore/data/FIGRIM")
    ...
    dataset = load_figrim_dataset(figrim_root, protocol="across", score_type="corrected_hit_rate")
    print(f"FIGRIM: {len(dataset.records)} records, {len(set(dataset.categories))} categories")
    ...
    # --- ResMem baseline ---
    resmem_cache = cache_dir / "resmem" / "figrim_targets_predictions.npz"
    ...
    # --- DINOv2 ---
    dinov2_cache = cache_dir / "dinov2" / "figrim_targets_all.npz"
    ...
    clip_bundle = clip_embedder.embed_split(
        dataset.records, split_name="figrim_targets_all", cache_dir=clip_cache,
    )
    ...
    # --- Summary ---
    print("\n=== FIGRIM across_category Summary ===")
    ...
    out = results_dir / "figrim_dinov2.json"
